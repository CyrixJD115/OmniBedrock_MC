...
@router.get("/")
async def get_properties(auth: str = Depends(verify_token)) -> list[dict]:
    return [e.model_dump() for e in _service.load()]


@router.get("/raw")
async def get_properties_raw(auth: str = Depends(verify_token)) -> PlainTextResponse:
    return PlainTextResponse(_service.get_raw())


@router.put("/raw")
async def save_properties_raw(body: dict, auth: str = Depends(verify_token)):
    text = body.get("text", "")
...
@router.put("/{key}")
async def update_property(key: str, body: dict, auth: str = Depends(verify_token)):
